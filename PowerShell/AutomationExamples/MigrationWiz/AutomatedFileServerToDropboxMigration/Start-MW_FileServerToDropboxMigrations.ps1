...
.DESCRIPTION
    This script will start migrations all migrations created by Create-MW_AzureBlobContainerToDropbox.ps1 under a MSPC Customer.
...
Function Get-CsvFile {
    Write-Host
    Write-Host -ForegroundColor yellow "ACTION: Select the CSV file to import the FileServer to DropBox MigrationWiz project."
...
Write-Host
$msg = "INFO: Retrieving FileServer to DropBox MigrationWiz projects..."
Write-Host $msg
Log-Write -Message $msg -LogFile $logFile

if($connectorsFromCSVFile -ne $null -and $connectorsFromCSVFile.Length -ge 1) {
    Write-Host -ForegroundColor Green "SUCCESS: 1 FileServer to DropBox MigrationWiz projects found:" 

        $connectorFromCSVFile = $connectorsFromCSVFile[0]
        Write-Host -Object "1-",$connectorFromCSVFile.ProjectName
}
else {
    $msg = "INFO: No FileServer to DropBox MigrationWiz projects found in the CSV file. Script aborted."
    Write-Host -ForegroundColor Red  $msg
    Log-Write -Message $msg -LogFile $logFile
    Exit
}
...
Write-Host
$msg = "INFO: Retrieving migrations from the FileServer to DropBox MigrationWiz project..."
Write-Host $msg
Log-Write -Message $msg -LogFile $logFile
...
foreach ($connector in $connectors) {
        ...
        if($mailboxesPage) {
            ...
            foreach($mailbox in $mailboxesPage) {
                if($connector.ProjectType -eq "Storage") {
                    Write-Progress -Activity ("Retrieving migrations from '$($connector.Name)' FileServer to DropBox MigrationWiz project") -Status $mailbox.ImportEmailAddress.ToLower()
                }
            }
    ...
    if($mailboxes -ne $null -and $mailboxes.Length -ge 1) {
        $msg = "SUCCESS: $($mailboxes.Length) migrations found." 
        Write-Host -ForegroundColor Green  $msg
        Log-Write -Message $msg -LogFile $logFile
    }
    else {
        $msg = "INFO: No migrations found. Script aborted." 
        Write-Host -ForegroundColor Red  $msg
        Log-Write -Message $msg -LogFile $logFile
        Exit
    }
...
do {
    $action = Menu-MigrationSubmission -AllMailboxes $allMailboxes -ProjectName "FS-DropBox-" 
	if($action -ne $null) {
			$action = Menu-MigrationSubmission -AllMailboxes $allMailboxes -ProjectName "FS-DropBox-" 
	}
	else {
